...
void efGroundUpdatePhysics(GObj *effect_gobj, s32 effect_id)
{
    ...
    ((sEFGroundActor.effect_data->effect_descs[effect_id].alt_high - sEFGroundActor.effect_data->effect_descs[effect_id].alt_low) * syUtilsRandCosmeticFloat()) +
...
void efGroundMakeEffectID(s32 effect_id)
{
    ...
    {
        sEFGroundActor.make_wait = syUtilsRandCosmeticIntRange(10000) + 6000;
    }
...
void EFGroundActorProcUpdate(GObj *gobj)
{
    ...
    if (sEFGroundActor.make_wait == 0)
    {
        ...
        if (sEFGroundActor.make_queue == 0)
        {
            param_id = syUtilsRandCosmeticIntRange(sEFGroundActor.effect_count);
            ...
            {
                sEFGroundActor.lr = (syUtilsRandCosmeticIntRange(2) == 0) ? -1 : +1;
            }
            sEFGroundActor.make_queue = (param + param_id)->make_queue;

            if (sEFGroundActor.make_queue != 0)
            {
                sEFGroundActor.make_queue += syUtilsRandCosmeticIntRange(2);
            }
        }
