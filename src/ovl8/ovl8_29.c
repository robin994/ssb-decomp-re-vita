#include "common.h"
#include <sys/develop.h>

#ifdef PORT
extern dbUnknownLinkStruct D_ovl8_8038DEC0;
extern dbFunction D_ovl8_8038DFA0;
extern dbFunction D_ovl8_8038E0F8;
extern dbUnknownLinkStruct D_ovl8_8038E120;
extern dbFunction D_ovl8_8038E188;
#else


extern sb32 func_ovl8_803718FC();
extern sb32 func_ovl8_80371930();
extern sb32 func_ovl8_80371960();
extern sb32 func_ovl8_80371968();
extern sb32 func_ovl8_80371970();
extern sb32 func_ovl8_80371978();
extern sb32 func_ovl8_80371980();
extern sb32 func_ovl8_803719A4();
extern sb32 func_ovl8_803719C8();
extern sb32 func_ovl8_80371A2C();
extern sb32 func_ovl8_80371AC4();
extern sb32 func_ovl8_80371ACC();
extern sb32 func_ovl8_80371AD8();
extern sb32 func_ovl8_80371B34();
extern sb32 func_ovl8_80371B98();
extern sb32 func_ovl8_80371BCC();
extern sb32 func_ovl8_80371C2C();
extern sb32 func_ovl8_80371C8C();
extern sb32 func_ovl8_80371D08();
extern sb32 func_ovl8_80371D64();
extern sb32 func_ovl8_80371D74();
extern sb32 func_ovl8_80371DC0();
extern sb32 func_ovl8_80371DD0();
extern sb32 func_ovl8_80371DDC();
extern sb32 func_ovl8_80371DE4();
extern sb32 func_ovl8_80371DF4();
extern sb32 func_ovl8_80371DFC();
extern sb32 func_ovl8_80371E04();
extern sb32 func_ovl8_80371E0C();
extern sb32 func_ovl8_80371E14();
extern sb32 func_ovl8_80371E1C();
extern sb32 func_ovl8_80371E24();
extern sb32 func_ovl8_80371E2C();
extern sb32 func_ovl8_80371E58();
extern sb32 func_ovl8_803721E8();
extern sb32 func_ovl8_80372224();
extern sb32 func_ovl8_80372250();
extern sb32 func_ovl8_803722A0();
extern sb32 func_ovl8_803722F0();
extern sb32 func_ovl8_80372348();
extern sb32 func_ovl8_80372358();
extern sb32 func_ovl8_80372360();
extern sb32 func_ovl8_803726CC();
extern sb32 func_ovl8_80373640();
extern sb32 func_ovl8_80373650();
extern sb32 func_ovl8_8037367C();
extern sb32 func_ovl8_80373684();
extern sb32 func_ovl8_8037368C();
extern sb32 func_ovl8_80373750();
extern sb32 func_ovl8_80373790();
extern sb32 func_ovl8_803737DC();
extern sb32 func_ovl8_80373930();
extern sb32 func_ovl8_80373980();
extern sb32 func_ovl8_80373988();
extern sb32 func_ovl8_80373990();
extern sb32 func_ovl8_80373A28();
extern sb32 func_ovl8_80373ABC();
extern sb32 func_ovl8_80373AC4();
extern sb32 func_ovl8_80373B28();
extern sb32 func_ovl8_80373B84();
extern sb32 func_ovl8_80373BD4();
extern sb32 func_ovl8_80373EA8();
extern sb32 func_ovl8_803743C0();
extern sb32 func_ovl8_803743D8();
extern sb32 func_ovl8_8037C344();
extern sb32 func_ovl8_8037C358();
extern sb32 func_ovl8_8037CA48();
extern sb32 func_ovl8_8038125C();
extern sb32 func_ovl8_80381274();
extern sb32 func_ovl8_803812BC();
extern sb32 func_ovl8_80381308();
extern sb32 func_ovl8_80381710();
extern sb32 func_ovl8_8038185C();
extern sb32 func_ovl8_80381908();
void func_ovl8_803817C0(void*, f32);
void func_ovl8_80385C70();
void func_ovl8_80385D34();
void func_ovl8_80385D54(s32, f32);
void func_ovl8_80385D88();

dbFunction D_ovl8_8038DEC0[] = {
	{0, NULL},
	{0, (sb32(*)())(func_ovl8_80385C70)},
	{0, func_ovl8_80373640},
	{0, (sb32(*)())(func_ovl8_80385D88)},
	{0, func_ovl8_80373650},
	{0, func_ovl8_8037367C},
	{0, func_ovl8_80373684},
	{0, func_ovl8_8037368C},
	{0, (sb32(*)())(func_ovl8_80385D54)},
	{0, func_ovl8_80373750},
	{0, func_ovl8_80373790},
	{0, func_ovl8_803737DC},
	{0, (sb32(*)())(func_ovl8_80385D34)},
	{0, func_ovl8_80373EA8},
	{0, func_ovl8_80373930},
	{0, func_ovl8_80373980},
	{0, func_ovl8_80373988},
	{0, func_ovl8_80373990},
	{0, func_ovl8_80373A28},
	{0, func_ovl8_80373ABC},
	{0, func_ovl8_80373AC4},
	{0, func_ovl8_80373B28},
	{0, func_ovl8_80373B84},
	{0, func_ovl8_803743D8},
	{0, func_ovl8_80373BD4},
	{0, func_ovl8_803743C0},
	{0, func_ovl8_8037CA48},
	{0, NULL},
};

dbFunction D_ovl8_8038DFA0[] = {
	{0, NULL},
	{(s16)0xFF28, (sb32(*)())(func_ovl8_80385C70)},
	{0, func_ovl8_803718FC},
	{0, func_ovl8_80371930},
	{0, func_ovl8_80371960},
	{0, func_ovl8_80371968},
	{0, func_ovl8_80371970},
	{0, func_ovl8_80371978},
	{0, func_ovl8_80371980},
	{0, func_ovl8_803719A4},
	{0, func_ovl8_803719C8},
	{0, func_ovl8_80371A2C},
	{(s16)0xFF28, (sb32(*)())(func_ovl8_80385D88)},
	{0, func_ovl8_80371AC4},
	{0, func_ovl8_80371ACC},
	{0, func_ovl8_80371AD8},
	{0, func_ovl8_80371B34},
	{0, func_ovl8_80371B98},
	{0, func_ovl8_80371BCC},
	{(s16)0xFF28, func_ovl8_80373650},
	{0, func_ovl8_80371C2C},
	{(s16)0xFF68, func_ovl8_80372224},
	{0, func_ovl8_80371C8C},
	{(s16)0xFF68, func_ovl8_80372358},
	{(s16)0xFF28, func_ovl8_80373640},
	{0, func_ovl8_80371D08},
	{0, func_ovl8_80371D64},
	{0, func_ovl8_80371D74},
	{0, func_ovl8_80371DC0},
	{0, func_ovl8_80371DD0},
	{0, func_ovl8_80371DDC},
	{0, func_ovl8_80371DE4},
	{(s16)0xFF68, func_ovl8_803726CC},
	{0, func_ovl8_80371DF4},
	{0, func_ovl8_80371DFC},
	{0, func_ovl8_80371E04},
	{0, func_ovl8_80371E0C},
	{0, func_ovl8_80371E14},
	{0, func_ovl8_80371E1C},
	{0, func_ovl8_80371E24},
	{0, func_ovl8_80371E2C},
	{0, func_ovl8_80371E58},
	{0, NULL},
};

dbFunction D_ovl8_8038E0F8[] = {
	{0, NULL},
	{(s16)0xFECC, (sb32(*)())(func_ovl8_80385C70)},
	{0, func_ovl8_8037C344},
	{0, func_ovl8_8037C358},
	{0, NULL},
};

dbFunction D_ovl8_8038E120[] = {
	{0, NULL},
	{(s16)0xFFC0, (sb32(*)())(func_ovl8_80385C70)},
	{(s16)0xFFC0, (sb32(*)())(func_ovl8_80385D88)},
	{0, func_ovl8_803721E8},
	{0, func_ovl8_80372224},
	{0, func_ovl8_80372250},
	{0, func_ovl8_803722A0},
	{0, func_ovl8_803722F0},
	{0, func_ovl8_80372348},
	{0, func_ovl8_80372358},
	{0, func_ovl8_80372360},
	{0, func_ovl8_803726CC},
	{0, NULL},
};

dbFunction D_ovl8_8038E188[] = {
	{0, NULL},
	{(s16)0xFF48, (sb32(*)())(func_ovl8_80385C70)},
	{0, func_ovl8_8038125C},
	{0, func_ovl8_80381274},
	{0, func_ovl8_803812BC},
	{0, func_ovl8_80381308},
	{0, func_ovl8_80381710},
	{0, (sb32(*)())(func_ovl8_803817C0)},
	{0, func_ovl8_8038185C},
	{0, func_ovl8_80381908},
	{0, NULL},
};
#endif

void func_ovl8_803817C0(void*, f32);
void func_ovl8_80373694(void*, f32);
dbUnknown5* func_ovl8_80385B74(dbUnknown5 *arg0, dbUnknownLinkStruct *arg1, dbUnknownLink *arg2, dbUnknown3* arg3, s32 arg4);

#ifdef PORT
// 0x80385A50
#else
// 80385A50
#endif
s32 func_ovl8_80385A50(s32 arg0, s32 arg1)
{
	s32 sp24;
	s32 temp_v0 = func_ovl8_803717A0(0x140);

	if (temp_v0 != 0)
	{
		sp24 = temp_v0;
		func_ovl8_80385B74(temp_v0, 0, 0, arg1, arg0);
	}
	else
		sp24 = 0;

	return sp24;
}

#ifdef PORT
// 0x80385AAC
#else
// 80385AAC
#endif
dbUnknown5* func_ovl8_80385AAC(dbUnknown5 *arg0, dbUnknownLinkStruct *arg1, dbUnknownLink *arg2) 
{
#ifdef PORT
    if ((arg0 != 0) || ((arg0 = (dbUnknown5 *) func_ovl8_803717A0(0x140)) != 0)) 
    {
        if (arg1 == NULL) 
        {
            arg1 = (dbUnknownLinkStruct *)&arg0->unk_dbunk5_0xD8;
            arg2 = (dbUnknownLink *)&arg0->unk_dbunk5_0x134;
            #line 37
            func_ovl8_803717E0(arg1);
            func_ovl8_8037C2D0(arg2);
            #line 44
        }
        func_ovl8_8037C710(arg0, arg1, arg2);
        func_ovl8_80381130(arg0->unk_dbunk5_0xB8);
        arg0->unk_dbunk5_0x30 = &D_ovl8_8038DEC0;
        arg1->db_func = &D_ovl8_8038DFA0;
        arg2->unk_dbunklink_0x8 = (dbUnknownLink *)&D_ovl8_8038E0F8;
        arg0->unk_dbunk5_0x4C = (dbUnknownLinkStruct *)&D_ovl8_8038E120,\
        arg0->unk_dbunk5_0xD4 = (s32)&D_ovl8_8038E188;
    }
    return arg0;
#else
	if ((arg0 != 0) || ((arg0 = (dbUnknown5 *) func_ovl8_803717A0(0x140)) != 0)) 
	{
		if (arg1 == NULL) 
		{
			arg1 = (dbUnknownLinkStruct *)&arg0->unk_dbunk5_0xD8;
			arg2 = (dbUnknownLink *)&arg0->unk_dbunk5_0x134;
			#line 228
			func_ovl8_803717E0(arg1);
			func_ovl8_8037C2D0(arg2);
			#line 235
		}
		func_ovl8_8037C710(arg0, arg1, arg2);
		func_ovl8_80381130(arg0->unk_dbunk5_0xB8);
		arg0->unk_dbunk5_0x30 = &D_ovl8_8038DEC0;
		arg1->db_func = &D_ovl8_8038DFA0;
		arg2->unk_dbunklink_0x8 = (dbUnknownLink *)&D_ovl8_8038E0F8;
		arg0->unk_dbunk5_0x4C = (dbUnknownLinkStruct *)&D_ovl8_8038E120,\
		arg0->unk_dbunk5_0xD4 = (s32)&D_ovl8_8038E188;
	}
	return arg0;
#endif
}

#ifdef PORT
// 0x80385B74
#else
// 80385B74
#endif
dbUnknown5* func_ovl8_80385B74(dbUnknown5 *arg0, dbUnknownLinkStruct *arg1, dbUnknownLink *arg2, dbUnknown3* arg3, s32 arg4) 
{
#ifdef PORT
    s32 pad;
    
    if ((arg0 != 0) || ((arg0 = (dbUnknown5 *) func_ovl8_803717A0(0x140)) != 0)) 
    {
        if (arg1 == NULL) 
        {
            arg1 = (dbUnknownLinkStruct *)&arg0->unk_dbunk5_0xD8;
            arg2 = (dbUnknownLink *)&arg0->unk_dbunk5_0x134;
            #line 64
            func_ovl8_803717E0(arg1);
            func_ovl8_8037C2D0(arg2);
            #line 71
        }
        
        func_ovl8_8037C7D4(arg0, arg1, arg2, arg3, arg3->unk_dbunk3_0x24->unk_dbunk3_0x0, arg3->unk_dbunk3_0x24->unk_dbunk3_0x4, arg4);
        func_ovl8_8038116C(arg0->unk_dbunk5_0xB8, arg3->unk_dbunk3_0x24->unk_dbunk3_0x8, arg0);
        
        arg0->unk_dbunk5_0x30 = &D_ovl8_8038DEC0;
        arg1->db_func = &D_ovl8_8038DFA0;
        arg2->unk_dbunklink_0x8 = (dbUnknownLink *)&D_ovl8_8038E0F8;
        arg0->unk_dbunk5_0x4C = (dbUnknownLinkStruct *)&D_ovl8_8038E120,\
        arg0->unk_dbunk5_0xD4 = (s32)&D_ovl8_8038E188;
    }
    
    return arg0;
#else
	s32 pad;
	
	if ((arg0 != 0) || ((arg0 = (dbUnknown5 *) func_ovl8_803717A0(0x140)) != 0)) 
	{
		if (arg1 == NULL) 
		{
			arg1 = (dbUnknownLinkStruct *)&arg0->unk_dbunk5_0xD8;
			arg2 = (dbUnknownLink *)&arg0->unk_dbunk5_0x134;
			#line 255
			func_ovl8_803717E0(arg1);
			func_ovl8_8037C2D0(arg2);
			#line 262
		}
		
		func_ovl8_8037C7D4(arg0, arg1, arg2, arg3, arg3->unk_dbunk3_0x24->unk_dbunk3_0x0, arg3->unk_dbunk3_0x24->unk_dbunk3_0x4, arg4);
		func_ovl8_8038116C(arg0->unk_dbunk5_0xB8, arg3->unk_dbunk3_0x24->unk_dbunk3_0x8, arg0);
		
		arg0->unk_dbunk5_0x30 = &D_ovl8_8038DEC0;
		arg1->db_func = &D_ovl8_8038DFA0;
		arg2->unk_dbunklink_0x8 = (dbUnknownLink *)&D_ovl8_8038E0F8;
		arg0->unk_dbunk5_0x4C = (dbUnknownLinkStruct *)&D_ovl8_8038E120,\
		arg0->unk_dbunk5_0xD4 = (s32)&D_ovl8_8038E188;
	}
	
	return arg0;
#endif
}

#ifdef PORT
// 0x80385C70
#else
// 80385C70
#endif
void func_ovl8_80385C70(dbUnknown5* arg0, s32 arg1)
{
#ifdef PORT
    if (arg0 != NULL)
    {
        arg0->unk_dbunk5_0x30 = &D_ovl8_8038DEC0,
        arg0->unk_dbunk5_0x38->db_func = &D_ovl8_8038DFA0;
        arg0->unk_dbunk5_0x34->unk_dbunklink_0x8 = &D_ovl8_8038E0F8;
        
        arg0->unk_dbunk5_0x4C = &D_ovl8_8038E120;\
        arg0->unk_dbunk5_0xD4 = &D_ovl8_8038E188;
#else
	if (arg0 != NULL)
	{
		arg0->unk_dbunk5_0x30 = &D_ovl8_8038DEC0,
		arg0->unk_dbunk5_0x38->db_func = &D_ovl8_8038DFA0;
		arg0->unk_dbunk5_0x34->unk_dbunklink_0x8 = &D_ovl8_8038E0F8;
		
		arg0->unk_dbunk5_0x4C = &D_ovl8_8038E120;\
		arg0->unk_dbunk5_0xD4 = &D_ovl8_8038E188;
#endif

#ifdef PORT
        #line 94
        func_ovl8_8038120C(arg0->unk_dbunk5_0xB8, 0);
        #line 101
        func_ovl8_8037C92C(arg0, 0);
#else
		#line 285
		func_ovl8_8038120C(arg0->unk_dbunk5_0xB8, 0);
		#line 292
		func_ovl8_8037C92C(arg0, 0);
#endif

#ifdef PORT
        if (arg1 != 0)
        {
            func_ovl8_8037C30C(arg0->unk_dbunk5_0x34, 0);
            func_ovl8_803718C4(arg0->unk_dbunk5_0x38, 0);
        }
        if (arg1 & 1)
        {
            func_ovl8_803717C0(arg0);
        }
    }
#else
		if (arg1 != 0)
		{
			func_ovl8_8037C30C(arg0->unk_dbunk5_0x34, 0);
			func_ovl8_803718C4(arg0->unk_dbunk5_0x38, 0);
		}
		if (arg1 & 1)
		{
			func_ovl8_803717C0(arg0);
		}
	}
#endif
}

#ifdef PORT
// 0x80385D34
#else
// 80385D34
#endif
void func_ovl8_80385D34(s32 arg0)
{
#ifdef PORT
    func_ovl8_80381308(arg0 + 0xB8);
#else
	func_ovl8_80381308(arg0 + 0xB8);
#endif
}

#ifdef PORT
// 0x80385D54
#else
// 80385D54
#endif
void func_ovl8_80385D54(s32 arg0, f32 arg1)
{
#ifdef PORT
    func_ovl8_803817C0(arg0 + 0xB8, arg1);
    func_ovl8_80373694(arg0, arg1);
#else
	func_ovl8_803817C0(arg0 + 0xB8, arg1);
	func_ovl8_80373694(arg0, arg1);
#endif
}

#ifdef PORT
// 0x80385D88
#else
// 80385D88
#endif
void func_ovl8_80385D88(dbUnknown5* arg0)
{
#ifdef PORT
    dbUnknownLinkStruct *var_v0;
    dbUnknownLinkStruct *var_v1;
#else
	dbUnknownLinkStruct *var_v0;
	dbUnknownLinkStruct *var_v1;
#endif

#ifdef PORT
    if ((arg0->unk_dbunk5_0x0 != 0.0f) && ((arg0->unk_dbunk5_0xD0 != 0) || (arg0->unk_dbunk5_0x1E != 0)))
    {
        var_v0 = arg0->unk_dbunk5_0xB0;
        var_v1 = (var_v0 != 0) ? var_v0 : arg0->unk_dbunk5_0xB4;
        var_v0 = var_v1;
    }
    else
    {
        var_v0 = arg0->unk_dbunk5_0xB4;
    }
#else
	if ((arg0->unk_dbunk5_0x0 != 0.0f) && ((arg0->unk_dbunk5_0xD0 != 0) || (arg0->unk_dbunk5_0x1E != 0)))
	{
		var_v0 = arg0->unk_dbunk5_0xB0;
		var_v1 = (var_v0 != 0) ? var_v0 : arg0->unk_dbunk5_0xB4;
		var_v0 = var_v1;
	}
	else
	{
		var_v0 = arg0->unk_dbunk5_0xB4;
	}
#endif

#ifdef PORT
    arg0->unk_dbunk5_0x48 = var_v0;
    func_ovl8_803725DC(&arg0->unk_dbunk5_0x40);
#else
	arg0->unk_dbunk5_0x48 = var_v0;
	func_ovl8_803725DC(&arg0->unk_dbunk5_0x40);
#endif
}
