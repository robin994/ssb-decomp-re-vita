#include "common.h"
#include <sys/develop.h>

#ifdef PORT
extern dbUnknownLinkStruct D_ovl8_8038DC00;
extern dbFunction D_ovl8_8038DCD8;
extern dbUnknownLink D_ovl8_8038DE30;
extern dbUnknownLinkStruct D_ovl8_8038DE58;
#else


extern sb32 func_ovl8_803718FC();
extern sb32 func_ovl8_80371930();
extern sb32 func_ovl8_80371960();
extern sb32 func_ovl8_80371968();
extern sb32 func_ovl8_80371970();
extern sb32 func_ovl8_80371978();
extern sb32 func_ovl8_80371980();
extern sb32 func_ovl8_803719A4();
extern sb32 func_ovl8_803719C8();
extern sb32 func_ovl8_80371A2C();
extern sb32 func_ovl8_80371ACC();
extern sb32 func_ovl8_80371AD8();
extern sb32 func_ovl8_80371B34();
extern sb32 func_ovl8_80371B98();
extern sb32 func_ovl8_80371BCC();
extern sb32 func_ovl8_80371C2C();
extern sb32 func_ovl8_80371C8C();
extern sb32 func_ovl8_80371D08();
extern sb32 func_ovl8_80371D64();
extern sb32 func_ovl8_80371D74();
extern sb32 func_ovl8_80371DC0();
extern sb32 func_ovl8_80371DD0();
extern sb32 func_ovl8_80371DDC();
extern sb32 func_ovl8_80371DE4();
extern sb32 func_ovl8_80371DF4();
extern sb32 func_ovl8_80371DFC();
extern sb32 func_ovl8_80371E04();
extern sb32 func_ovl8_80371E0C();
extern sb32 func_ovl8_80371E14();
extern sb32 func_ovl8_80371E1C();
extern sb32 func_ovl8_80371E24();
extern sb32 func_ovl8_80371E2C();
extern sb32 func_ovl8_80371E58();
extern sb32 func_ovl8_803721E8();
extern sb32 func_ovl8_80372224();
extern sb32 func_ovl8_80372250();
extern sb32 func_ovl8_803722A0();
extern sb32 func_ovl8_803722F0();
extern sb32 func_ovl8_80372348();
extern sb32 func_ovl8_80372358();
extern sb32 func_ovl8_80372360();
extern sb32 func_ovl8_803726CC();
extern sb32 func_ovl8_80373650();
extern sb32 func_ovl8_8037367C();
extern sb32 func_ovl8_80373684();
extern sb32 func_ovl8_8037368C();
extern sb32 func_ovl8_80373694();
extern sb32 func_ovl8_80373750();
extern sb32 func_ovl8_80373790();
extern sb32 func_ovl8_803737DC();
extern sb32 func_ovl8_80373828();
extern sb32 func_ovl8_8037383C();
extern sb32 func_ovl8_80373980();
extern sb32 func_ovl8_80373988();
extern sb32 func_ovl8_80373990();
extern sb32 func_ovl8_80373A28();
extern sb32 func_ovl8_80373ABC();
extern sb32 func_ovl8_80373AC4();
extern sb32 func_ovl8_80373B28();
extern sb32 func_ovl8_80373B84();
extern sb32 func_ovl8_80373BC0();
extern sb32 func_ovl8_80373BD4();
extern sb32 func_ovl8_8037C344();
extern sb32 func_ovl8_8037C358();
extern sb32 func_ovl8_803822D8();
void func_ovl8_803858B0();
void func_ovl8_8038597C();
void func_ovl8_8038599C();
void func_ovl8_80385A1C();

dbFunction D_ovl8_8038DC00[] = {
	{0, NULL},
	{0, (sb32(*)())(func_ovl8_803858B0)},
	{0, (sb32(*)())(func_ovl8_8038599C)},
	{0, (sb32(*)())(func_ovl8_8038597C)},
	{0, func_ovl8_80373650},
	{0, func_ovl8_8037367C},
	{0, func_ovl8_80373684},
	{0, func_ovl8_8037368C},
	{0, func_ovl8_80373694},
	{0, func_ovl8_80373750},
	{0, func_ovl8_80373790},
	{0, func_ovl8_803737DC},
	{0, func_ovl8_80373828},
	{0, func_ovl8_8037383C},
	{0, func_ovl8_803822D8},
	{0, func_ovl8_80373980},
	{0, func_ovl8_80373988},
	{0, func_ovl8_80373990},
	{0, func_ovl8_80373A28},
	{0, func_ovl8_80373ABC},
	{0, func_ovl8_80373AC4},
	{0, func_ovl8_80373B28},
	{0, func_ovl8_80373B84},
	{0, func_ovl8_80373BC0},
	{0, func_ovl8_80373BD4},
	{0, (sb32(*)())(func_ovl8_80385A1C)},
	{0, NULL},
};

dbFunction D_ovl8_8038DCD8[] = {
	{0, NULL},
	{(s16)0xFFAC, (sb32(*)())(func_ovl8_803858B0)},
	{0, func_ovl8_803718FC},
	{0, func_ovl8_80371930},
	{0, func_ovl8_80371960},
	{0, func_ovl8_80371968},
	{0, func_ovl8_80371970},
	{0, func_ovl8_80371978},
	{0, func_ovl8_80371980},
	{0, func_ovl8_803719A4},
	{0, func_ovl8_803719C8},
	{0, func_ovl8_80371A2C},
	{(s16)0xFFAC, (sb32(*)())(func_ovl8_8038597C)},
	{(s16)0xFFAC, (sb32(*)())(func_ovl8_80385A1C)},
	{0, func_ovl8_80371ACC},
	{0, func_ovl8_80371AD8},
	{0, func_ovl8_80371B34},
	{0, func_ovl8_80371B98},
	{0, func_ovl8_80371BCC},
	{(s16)0xFFAC, func_ovl8_80373650},
	{0, func_ovl8_80371C2C},
	{(s16)0xFFEC, func_ovl8_80372224},
	{0, func_ovl8_80371C8C},
	{(s16)0xFFEC, func_ovl8_80372358},
	{(s16)0xFFAC, (sb32(*)())(func_ovl8_8038599C)},
	{0, func_ovl8_80371D08},
	{0, func_ovl8_80371D64},
	{0, func_ovl8_80371D74},
	{0, func_ovl8_80371DC0},
	{0, func_ovl8_80371DD0},
	{0, func_ovl8_80371DDC},
	{0, func_ovl8_80371DE4},
	{(s16)0xFFEC, func_ovl8_803726CC},
	{0, func_ovl8_80371DF4},
	{0, func_ovl8_80371DFC},
	{0, func_ovl8_80371E04},
	{0, func_ovl8_80371E0C},
	{0, func_ovl8_80371E14},
	{0, func_ovl8_80371E1C},
	{0, func_ovl8_80371E24},
	{0, func_ovl8_80371E2C},
	{0, func_ovl8_80371E58},
	{0, NULL},
};

dbFunction D_ovl8_8038DE30[] = {
	{0, NULL},
	{(s16)0xFF4C, (sb32(*)())(func_ovl8_803858B0)},
	{0, func_ovl8_8037C344},
	{0, func_ovl8_8037C358},
	{0, NULL},
};

dbFunction D_ovl8_8038DE58[] = {
	{0, NULL},
	{(s16)0xFFC0, (sb32(*)())(func_ovl8_803858B0)},
	{(s16)0xFFC0, (sb32(*)())(func_ovl8_8038597C)},
	{0, func_ovl8_803721E8},
	{0, func_ovl8_80372224},
	{0, func_ovl8_80372250},
	{0, func_ovl8_803722A0},
	{0, func_ovl8_803722F0},
	{0, func_ovl8_80372348},
	{0, func_ovl8_80372358},
	{0, func_ovl8_80372360},
	{0, func_ovl8_803726CC},
	{0, NULL},
};
#endif

dbUnknown5* func_ovl8_80385758(dbUnknown5* arg0, dbUnknownLinkStruct* arg1, dbUnknownLink* arg2, DBMenu *arg3, s32 arg4);

extern void func_ovl8_803724B4(void *, void *, void *, s32, s32);

#ifdef PORT
// 0x80385640
#else
// 80385640
#endif
s32 func_ovl8_80385640(s32 arg0, s32 arg1)
{
	s32 temp_v0;
	s32 var_v1;

	temp_v0 = func_ovl8_803717A0(0xC0);
	if (temp_v0 != 0)
	{
		func_ovl8_80385758(temp_v0, 0, 0, arg1, arg0);
		var_v1 = temp_v0;
	}
	else
		var_v1 = 0;

	return var_v1;
}

#ifdef PORT
// 0x8038569C
#else
// 8038569C
#endif
dbUnknown5* func_ovl8_8038569C(dbUnknown5* arg0, dbUnknownLinkStruct* arg1, dbUnknownLink* arg2)
{
#ifdef PORT
    if (arg0 != NULL || ((arg0 = func_ovl8_803717A0(0xC0)) != NULL))
    {
        if (arg1 == NULL)
        {
            arg1 = &arg0->unk_dbunk5_0x54;\
            arg2 = &arg0->unk_dbunk5_0xB4;
            #line 37
            func_ovl8_803717E0(arg1);
            func_ovl8_8037C2D0(arg2);
            #line 44
        }
#else
	if (arg0 != NULL || ((arg0 = func_ovl8_803717A0(0xC0)) != NULL))
	{
		if (arg1 == NULL)
		{
			arg1 = &arg0->unk_dbunk5_0x54;\
			arg2 = &arg0->unk_dbunk5_0xB4;
			#line 204
			func_ovl8_803717E0(arg1);
			func_ovl8_8037C2D0(arg2);
			#line 211
		}
#endif

#ifdef PORT
        func_ovl8_803820B0(arg0, arg1, arg2);
        func_ovl8_80372430(&arg0->unk_dbunk5_0x40, arg1);
#else
		func_ovl8_803820B0(arg0, arg1, arg2);
		func_ovl8_80372430(&arg0->unk_dbunk5_0x40, arg1);
#endif

#ifdef PORT
        arg0->unk_dbunk5_0x30 = &D_ovl8_8038DC00,
        arg1->db_func = &D_ovl8_8038DCD8;
        arg2->unk_dbunklink_0x8 = &D_ovl8_8038DE30;
        arg0->unk_dbunk5_0x4C = &D_ovl8_8038DE58;
    }
#else
		arg0->unk_dbunk5_0x30 = &D_ovl8_8038DC00,
		arg1->db_func = &D_ovl8_8038DCD8;
		arg2->unk_dbunklink_0x8 = &D_ovl8_8038DE30;
		arg0->unk_dbunk5_0x4C = &D_ovl8_8038DE58;
	}
#endif

#ifdef PORT
    return arg0;
#else
	return arg0;
#endif
}

#ifdef PORT
// 0x80385758
#else
// 80385758
#endif
dbUnknown5* func_ovl8_80385758(dbUnknown5* arg0, dbUnknownLinkStruct* arg1, dbUnknownLink* arg2, DBMenu *arg3, s32 arg4)
{
#ifdef PORT
    s32 sp44;
    s32 sp40;
    s32 sp3C;
    u16 temp_v1;
    
    if (arg0 != NULL || ((arg0 = func_ovl8_803717A0(0xC0)) != NULL))
    {
        if (arg1 == NULL)
        {
            arg1 = &arg0->unk_dbunk5_0x54;\
            arg2 = &arg0->unk_dbunk5_0xB4;
            #line 69
            func_ovl8_803717E0(arg1);
            func_ovl8_8037C2D0(arg2);
            #line 76
        }
#else
	s32 sp44;
	s32 sp40;
	s32 sp3C;
	u16 temp_v1;
	
	if (arg0 != NULL || ((arg0 = func_ovl8_803717A0(0xC0)) != NULL))
	{
		if (arg1 == NULL)
		{
			arg1 = &arg0->unk_dbunk5_0x54;\
			arg2 = &arg0->unk_dbunk5_0xB4;
			#line 236
			func_ovl8_803717E0(arg1);
			func_ovl8_8037C2D0(arg2);
			#line 243
		}
#endif

#ifdef PORT
        func_ovl8_8038215C(arg0, arg1, arg2, arg3, arg4, arg3->unk_dbmenu_0x24);
        func_ovl8_803724B4(&arg0->unk_dbunk5_0x40, arg1, arg3, arg4, 0);
#else
		func_ovl8_8038215C(arg0, arg1, arg2, arg3, arg4, arg3->unk_dbmenu_0x24);
		func_ovl8_803724B4(&arg0->unk_dbunk5_0x40, arg1, arg3, arg4, 0);
#endif

#ifdef PORT
        arg0->unk_dbunk5_0x30 = &D_ovl8_8038DC00,
        arg1->db_func = &D_ovl8_8038DCD8;
        arg2->unk_dbunklink_0x8 = &D_ovl8_8038DE30;
        arg0->unk_dbunk5_0x4C = &D_ovl8_8038DE58;
        temp_v1 = arg3->unk_dbmenu_0x10;
        
        if (temp_v1 == 2) 
        {
            sp3C = 2;
        } 
        else if (temp_v1 == 5)
        {
            sp3C = 3;
        }
        
        sp44 = (s32) arg3->position.w;
        sp40 = (s32) arg3->position.h;
        func_ovl8_8037B85C(sp3C, &sp44, &sp40);
        arg0->unk_dbunk5_0xB0 = func_ovl8_8037ACAC(arg3->position.w, arg3->position.h, sp3C, sp44, sp40);
        arg0->unk_dbunk5_0x48 = arg0->unk_dbunk5_0xB0;
    }
#else
		arg0->unk_dbunk5_0x30 = &D_ovl8_8038DC00,
		arg1->db_func = &D_ovl8_8038DCD8;
		arg2->unk_dbunklink_0x8 = &D_ovl8_8038DE30;
		arg0->unk_dbunk5_0x4C = &D_ovl8_8038DE58;
		temp_v1 = arg3->unk_dbmenu_0x10;
		
		if (temp_v1 == 2) 
		{
			sp3C = 2;
		} 
		else if (temp_v1 == 5)
		{
			sp3C = 3;
		}
		
		sp44 = (s32) arg3->position.w;
		sp40 = (s32) arg3->position.h;
		func_ovl8_8037B85C(sp3C, &sp44, &sp40);
		arg0->unk_dbunk5_0xB0 = func_ovl8_8037ACAC(arg3->position.w, arg3->position.h, sp3C, sp44, sp40);
		arg0->unk_dbunk5_0x48 = arg0->unk_dbunk5_0xB0;
	}
#endif

#ifdef PORT
    return arg0;
#else
	return arg0;
#endif
}

#ifdef PORT
// 0x803858B0
#else
// 803858B0
#endif
void func_ovl8_803858B0(dbUnknown5* arg0, s32 arg1)
{
#ifdef PORT
    if (arg0 != NULL)
    {
        arg0->unk_dbunk5_0x30 = &D_ovl8_8038DC00,
        arg0->unk_dbunk5_0x38->db_func = &D_ovl8_8038DCD8;
        arg0->unk_dbunk5_0x34->unk_dbunklink_0x8 = &D_ovl8_8038DE30;
        arg0->unk_dbunk5_0x4C = &D_ovl8_8038DE58;
#else
	if (arg0 != NULL)
	{
		arg0->unk_dbunk5_0x30 = &D_ovl8_8038DC00,
		arg0->unk_dbunk5_0x38->db_func = &D_ovl8_8038DCD8;
		arg0->unk_dbunk5_0x34->unk_dbunklink_0x8 = &D_ovl8_8038DE30;
		arg0->unk_dbunk5_0x4C = &D_ovl8_8038DE58;
#endif

#ifdef PORT
        if (arg0->unk_dbunk5_0xB0 != 0) {
            func_ovl8_8037B3E4(arg0->unk_dbunk5_0xB0);
        }
#else
		if (arg0->unk_dbunk5_0xB0 != 0) {
			func_ovl8_8037B3E4(arg0->unk_dbunk5_0xB0);
		}
#endif

#ifdef PORT
        func_ovl8_80372554(&arg0->unk_dbunk5_0x40, 0);
        func_ovl8_80382224(arg0, 0);
#else
		func_ovl8_80372554(&arg0->unk_dbunk5_0x40, 0);
		func_ovl8_80382224(arg0, 0);
#endif

#ifdef PORT
        if (arg1 != 0)
        {
            func_ovl8_8037C30C(arg0->unk_dbunk5_0x34, 0);
            func_ovl8_803718C4(arg0->unk_dbunk5_0x38, 0);
        }
        if (arg1 & 1)
        {
            func_ovl8_803717C0(arg0);
        }
    }
#else
		if (arg1 != 0)
		{
			func_ovl8_8037C30C(arg0->unk_dbunk5_0x34, 0);
			func_ovl8_803718C4(arg0->unk_dbunk5_0x38, 0);
		}
		if (arg1 & 1)
		{
			func_ovl8_803717C0(arg0);
		}
	}
#endif
}

#ifdef PORT
// 0x8038597C
#else
// 8038597C
#endif
void func_ovl8_8038597C(s32 arg0)
{
#ifdef PORT
    func_ovl8_803725DC(arg0 + 0x40);
#else
	func_ovl8_803725DC(arg0 + 0x40);
#endif
}

#ifdef PORT
// 0x8038599C
#else
// 8038599C
#endif
void func_ovl8_8038599C(dbFunction **arg0)
{
#ifdef PORT
    s32 sp2C;
    dbFunction *temp_v0;
    s32 sp24;
    dbFunction *temp_v0_3;
#else
	s32 sp2C;
	dbFunction *temp_v0;
	s32 sp24;
	dbFunction *temp_v0_3;
#endif

#ifdef PORT
    temp_v0 = arg0[19];
    sp2C = temp_v0[9].unk_dbfunc_0x4(temp_v0[9].unk_dbfunc_0x0 + (char*) arg0 + 0x40);
#else
	temp_v0 = arg0[19];
	sp2C = temp_v0[9].unk_dbfunc_0x4(temp_v0[9].unk_dbfunc_0x0 + (char*) arg0 + 0x40);
#endif

#ifdef PORT
    if (sp2C == 0)
        return;
#else
	if (sp2C == 0)
		return;
#endif

#ifdef PORT
    temp_v0_3 = arg0[19];
    temp_v0_3[4].unk_dbfunc_0x4(temp_v0_3[4].unk_dbfunc_0x0 + (char*) arg0 + 0x40, &sp24);
    func_ovl8_80377AEC(sp2C, &sp24, arg0[14] + 3, 4);
    func_ovl8_80382354(arg0);
#else
	temp_v0_3 = arg0[19];
	temp_v0_3[4].unk_dbfunc_0x4(temp_v0_3[4].unk_dbfunc_0x0 + (char*) arg0 + 0x40, &sp24);
	func_ovl8_80377AEC(sp2C, &sp24, arg0[14] + 3, 4);
	func_ovl8_80382354(arg0);
#endif
}

#ifdef PORT
// 0x80385A1C
#else
// 80385A1C
#endif
void func_ovl8_80385A1C(dbUnknownS38* arg0)
{
#ifdef PORT
    dbFunction* temp_v0 = arg0->unk_dbunks38_0x30;
    temp_v0[2].unk_dbfunc_0x4(temp_v0[2].unk_dbfunc_0x0 + (uintptr_t)arg0);
#else
	dbFunction* temp_v0 = arg0->unk_dbunks38_0x30;
	temp_v0[2].unk_dbfunc_0x4(temp_v0[2].unk_dbfunc_0x0 + (uintptr_t)arg0);
#endif
}
