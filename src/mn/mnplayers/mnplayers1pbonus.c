...
#ifdef PORT
		/* Widescreen: pre-divide world-x so the 3D fighter lands at its
		 * 4:3 authored NDC position vs the 2D panel UI. Same pattern as
		 * the VS CSS fix. */
		{
...
{
	dMNPlayers1PBonusVideoSetup.zbuffer = SYVIDEO_ZBUFFER_START(320, 240, 0, 10, u16);
	...
	syTaskmanStartTask(&dMNPlayers1PBonusTaskmanSetup);
}
